...
void sprintf_buffer(const char* buffer, int size){
    int i;
    for(i=0;i<size;i++){
        if(isprint(buffer[i])){
            printf("%c", buffer[i]);
...
int main(int argc, char* argv[]){
    ...
    if(sd<0){
        printf("Could not create not socket: %m\n");
        return 1;
    }
    ...
    printf("New socket created woth sd %d\n", sd);
    ...
    if(setsockopt(sd, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on))){
        printf("Could not set socket %d option for resusablilty: %m\n", sd);
        close(sd);
        return 4;
    }
    ...
    if(listen(sd, SOMAXCONN)){
        printf("COuld not start listening on server socker %d: %m\n",sd);
        goto cleanup;
    }else{
    ...
    for(;;){
        printf("Starting epoll_wiat on %d fds \n", pollsize);
            ...
            if(events& EPOLLIN){
                if(fd==sd){
                    ...
                    if(inet_ntop(AF_INET, &peeraddr.sin_addr.s_addr, buffer, sizeof(buffer))!=NULL){
                        ...
                        printf("Failed to convert address from binary to text from: %m\n");
                    }
